...
export class PlayerGirl2 {
    ...
    constructor(scene: Phaser.Scene & { engine: any; generator?: any }, gridX: number, gridY: number, bodyAtlasKey: string, headAtlasKey: string) {
        this.scene = scene;
    ...
    getBodyFrame(dir: string, index: number) {
        return `body-walk-dir-${dir}-${index}.png`;
    }

    getHeadFrame(dir: string) {
        return `head-gp-2-dir-${dir}.png`;
    }

    setupAnimations(bodyAtlasKey: string) {
    ...
    getHeadNudgeFromFacing(facing: { dir: string; flipX: boolean }) {
