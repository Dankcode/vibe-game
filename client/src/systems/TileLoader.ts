...
import type { IsometricEngine } from './IsometricEngine';
...
export class TileLoader {
    ...
    activeTiles: Map<string, any>;
    loadQueue: Array<{ x: number; y: number; key: string }>;
    isLoading: boolean;
    currentChunkX: number;
    currentChunkY: number;

    constructor(scene: Phaser.Scene, engine: IsometricEngine, config: { chunkSize?: number; loadRadius?: number; unloadRadius?: number } = {}) {
        this.scene = scene;
        this.engine = engine;
     ...
     */
    update(playerGridX: number, playerGridY: number) {
     ...
     */
    createTile(gridX: number, gridY: number, tileId: number, elevation: number) {
        /*
     ...
     */
    createObject(gridX: number, gridY: number, type: string, elevation: number) {
        const pos = this.engine.toScreen(gridX, gridY, elevation);
     ...
     */
    async fetchChunkData(cx: number, cy: number) {
        // Try server first
        try {
     ...
     */
    getTile(gridX: number, gridY: number) {
