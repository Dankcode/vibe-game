...
async function loadTownEntries(sourceDir, manifest, activeBurgIds) {
    ...
    const files = (Array.isArray(manifest.files?.towns) ? manifest.files.towns : [])
        .filter((relativeFile) => {
            ...
            return active.has(manifestBurg?.burgId || inferredId);
        });
...
function assertCompleteActiveTownEntries(entries, activeBurgIds) {
    const expected = [...activeBurgIds].map(Number).sort((left, right) => left - right);
    const actual = entries.map((entry) => Number(entry.burgId)).sort((left, right) => left - right);
    if (actual.length !== expected.length || actual.some((burgId, index) => burgId !== expected[index])) {
        throw new Error(
            `Active town vector inputs must match [${expected.join(', ')}]; received [${actual.join(', ')}].`
        );
    }
}
...
async function writeTownVectorOutputs(vectorPackage) {
    ...
    await removeStaleTownVectorOutputs(new Set(vectorPackage.towns.map((town) => town.burgId)));
...
async function removeStaleTownVectorOutputs(activeBurgIds) {
    const entries = await readdir(OUTPUT_ASSET_DIR, { withFileTypes: true });
    await Promise.all(entries.flatMap((entry) => {
        if (!entry.isFile()) return [];
        const match = entry.name.match(/^burg-(\d+)\.(?:vector\.json|svg)$/);
        if (!match || activeBurgIds.has(Number(match[1]))) return [];
        return [unlink(path.join(OUTPUT_ASSET_DIR, entry.name))];
    }));
}
...
async function main(argv = process.argv.slice(2)) {
    ...
    const entries = await loadTownEntries(sourceDir, manifest, activeBurgIds);
    assertCompleteActiveTownEntries(entries, activeBurgIds);
